...
listener http:Listener serverPushBackendEP = new(serverPushTestPort2, { httpVersion: http:HTTP_2_0 });

http:Client serverPushClient = check new("http://localhost:" + serverPushTestPort1.toString());
http:Client backendClientEP = check new("http://localhost:" + serverPushTestPort2.toString(), { httpVersion: http:HTTP_2_0 });
