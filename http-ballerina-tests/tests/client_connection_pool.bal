...
function testGlobalPoolConfig() {
    ...
    var poolConfig = clients[0].config?.poolConfig;
    if (poolConfig is http:PoolConfiguration) {
        test:assertEquals(poolConfig.toString(), "");
    }
    poolConfig = clients[1].config?.poolConfig;
    if (poolConfig is http:PoolConfiguration) {
        test:assertEquals(poolConfig.toString(), "");
    }
    poolConfig = clients[2].config?.poolConfig;
    if (poolConfig is http:PoolConfiguration) {
        test:assertEquals(poolConfig.toString(), "");
    }
}
