...
listener http:Listener compressionTestEP = checkpanic new(CompressionConfigTest);
http:Client compressionClient = checkpanic new("http://localhost:" + CompressionConfigTest.toString());
