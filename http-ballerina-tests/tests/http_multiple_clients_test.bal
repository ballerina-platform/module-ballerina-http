...
listener http:Listener multipleClientListener1 = new(multipleClientTestPort1, { httpVersion: http:HTTP_2_0 });
listener http:Listener multipleClientListener2 = new(multipleClientTestPort2, { httpVersion: http:HTTP_2_0 });

http:Client multipleClientTestClient = check new("http://localhost:" + multipleClientTestPort1.toString());

http:Client h2WithPriorKnowledgeClient = check new("http://localhost:" + multipleClientTestPort2.toString(), { httpVersion: http:HTTP_2_0, http2Settings: {
        http2PriorKnowledge: true }, poolConfig: {} });

http:Client h1Client = check new("http://localhost:" + multipleClientTestPort2.toString(), { httpVersion: http:HTTP_1_1, poolConfig: {}});
