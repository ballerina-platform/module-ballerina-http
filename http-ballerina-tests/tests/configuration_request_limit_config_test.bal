...
function testInvalidHeaderLength() {
    ...
    if (response is http:ClientRequestError) {
        ...
        test:assertEquals(response.detail().statusCode, 431, msg = "Found unexpected output");
    } else {
        test:assertFail(msg = "Found unexpected output type: http:Response");
    }
}
...
function testInvalidPayloadSize() {
    ...
    if (response is http:ClientRequestError) {
        ...
        test:assertEquals(response.detail().statusCode, 413, msg = "Found unexpected output");
    } else {
        test:assertFail(msg = "Found unexpected output type: http:Response");
    }
}
