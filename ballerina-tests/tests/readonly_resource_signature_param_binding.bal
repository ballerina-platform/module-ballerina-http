...
listener http:Listener readonlyParamEP = new(readonlyQueryTestPort);
final http:Client readonlyParamClient = check new("http://localhost:" + readonlyQueryTestPort.toString());
...
function testReadonlyTypeWithPayloadStringArr() returns error? {
    ...
    assertJsonValue(response, "status", "readonly");
    assertJsonValue(response, "value", j);
}
...
function testReadonlyTypeWithPayloadMapJson() returns error? {
    ...
    assertJsonValue(response, "status", "readonly");
    assertJsonValue(response, "value", {"name":"hello go","team":"ba #ller @na"});
}
...
function testReadonlyTypeWithPayloadXML() returns error? {
    ...
    assertJsonValue(response, "status", "readonly");
    assertJsonValue(response, "value", "xml");
}
...
function testReadonlyTypeWithPayloadByteArr() returns error? {
    ...
    assertJsonValue(response, "status", "readonly");
    assertJsonValue(response, "value", "WSO2");
}
