...
public function testHttp2ListenerStop2() returns error? {
    http:Listener serviceEP = check new (http2ListenerStopTest2);
    check serviceEP.attach(new HelloService());
    check serviceEP.'start();

    http:Client client1 = check new ("localhost:" + http2ListenerStopTest2.toString(), http2Settings = { http2PriorKnowledge: true });
    string|error response = client1->/hello;
    if response is string {
        test:assertEquals(response, "Hello");
    } else {
    ...
    http:Client client2 = check new ("localhost:" + http2ListenerStopTest2.toString(), http2Settings = { http2PriorKnowledge: true });
    response = client2->/hello;
    if response is error {
        test:assertEquals(response.message(), "Something wrong with the connection");
    } else {
