...
int http2ResponseLimitsTestPort1 = getHttp2Port(responseLimitsTestPort1);
int http2ResponseLimitsTestPort2 = getHttp2Port(responseLimitsTestPort2);
...
function testHttp2ValidHeaderLengthOfResponse() returns error? {
    ...
    test:assertEquals(response.statusCode, 200, msg = "Found unexpected output");
    assertHeaderValue(check response.getHeader(X_HEADER), "Validated");
    assertTextPayload(response.getTextPayload(), "Hello World!!!");
}
