...
function testCallerRequestHeaderPreservation() returns error? {
    ...
    if response is http:Response {
        ...
        common:assertHeaderValue(check response.getHeader(callerReqHeader), "First Request");
        test:assertFalse(response.hasHeader(common:IF_NONE_MATCH));
        test:assertFalse(response.hasHeader(common:IF_MODIFIED_SINCE));
    ...
    if response is http:Response {
        test:assertEquals(response.statusCode, 200, msg = "Found unexpected output");
        common:assertHeaderValue(check response.getHeader(callerReqHeader), "Third Request");
        test:assertFalse(response.hasHeader(common:IF_NONE_MATCH));
        test:assertFalse(response.hasHeader(common:IF_MODIFIED_SINCE));
        common:assertHeaderValue(check response.getHeader(common:CONTENT_TYPE), common:APPLICATION_JSON);
        common:assertJsonPayload(response.getJsonPayload(), cachingPayload);
...
function testCallerRequestHeaderPreservation2() {
    http:Response|error response = cachingProxyTestClient->get("/validation-request", {[common:IF_NONE_MATCH] : "c854ce2c"});
    if response is http:Response {
