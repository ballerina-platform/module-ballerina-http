...
listener http:Listener failoverEP02 = new(9302, httpVersion = http:HTTP_1_1);

// Create an endpoint with port 8082 for the mock backend services.
listener http:Listener backendEP02 = new(8082, httpVersion = http:HTTP_1_1);

// Define the failover client end point to call the backend services.
final http:FailoverClient foBackendEP02 = check new(
...
final http:FailoverClient foBackendFailureEP02 = check new(
    httpVersion = http:HTTP_1_1,
    timeout = 5,
    failoverCodes = [501, 502, 503],
    interval = 5,
...
final http:FailoverClient foStatusCodesEP02 = check new(
    httpVersion = http:HTTP_1_1,
    timeout = 5,
    failoverCodes = [501, 502, 503],
    interval = 5,
...
service /failoverDemoService02 on failoverEP02 {
    resource function 'default invokeAllFailureEndpoint02(http:Caller caller, http:Request request) {
        http:Response|error backendRes = foBackendEP02->forward("/", request);
    ...
    resource function 'default invokeAllFailureEndpoint(http:Caller caller, http:Request request) {
        http:Response|error backendRes = foBackendFailureEP02->forward("/", request);
        if backendRes is http:Response {
            error? responseToCaller = caller->respond(backendRes);
            if responseToCaller is error {
                // log:printError("Error sending response", 'error = responseToCaller);
    ...
    resource function 'default invokeAllFailureStatusCodesEndpoint(http:Caller caller, http:Request request) {
        http:Response|error backendRes = foStatusCodesEP02->forward("/", request);
        if backendRes is http:Response {
            error? responseToCaller = caller->respond(backendRes);
            if responseToCaller is error {
                // log:printError("Error sending response", 'error = responseToCaller);
