...
function testHeaderUnavailability() returns error? {
    ...
    if response is http:Response {
        ...
        test:assertEquals(check response.getTextPayload(), "no header value found for 'foo'");
    } else {
        test:assertFail(msg = "Found unexpected output type: " + response.message());
    }
}
...
function testHeaderRecordParamWithCastingError() returns error? {
    ...
    test:assertEquals(response.statusCode, 400);
    test:assertEquals(check response.getTextPayload(), "header binding failed for parameter: 'x-rate-limit-remaining'");
}
...
function testHeaderParamsCastingError() returns error? {
    http:Response response = check headerBindingClient->get("/headerRecord/ofOtherNilableHeaders",
        {"iid": "hello"});
    test:assertEquals(response.statusCode, 400);
    test:assertEquals(check response.getTextPayload(), "header binding failed for parameter: 'iid'");

    response = check headerBindingClient->get("/headerRecord/ofOtherNilableHeaders",
        {"fid": "hello"});
    test:assertEquals(response.statusCode, 400);
    test:assertEquals(check response.getTextPayload(), "header binding failed for parameter: 'fid'");

    response = check headerBindingClient->get("/headerRecord/ofOtherNilableHeaders",
        {"did": "hello"});
    test:assertEquals(response.statusCode, 400);
    test:assertEquals(check response.getTextPayload(), "header binding failed for parameter: 'did'");

    response = check headerBindingClient->get("/headerRecord/ofOtherNilableHeaders",
        {"iaid": ["3", "5", "8", "hello"]});
    test:assertEquals(response.statusCode, 400);
    test:assertEquals(check response.getTextPayload(), "header binding failed for parameter: 'iaid'");

    response = check headerBindingClient->get("/headerRecord/ofOtherNilableHeaders",
        {"faid": ["3.445", "hello", "5.667", "8.206"]});
    test:assertEquals(response.statusCode, 400);
    test:assertEquals(check response.getTextPayload(), "header binding failed for parameter: 'faid'");

    response = check headerBindingClient->get("/headerRecord/ofOtherNilableHeaders",
        {"daid": ["3.4", "5.6", "hello", "8"]});
    test:assertEquals(response.statusCode, 400);
    test:assertEquals(check response.getTextPayload(), "header binding failed for parameter: 'daid'");
}
